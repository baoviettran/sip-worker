...
**Date:** 2026-08-03
**Status:** Approved for v1 (implementation-boundary revision applied)
...
- SIP CRLF keepalive for UDP/TCP. Node WebSocket protocol ping/pong and a
  browser-compatible SIP `OPTIONS` liveness strategy are in v1.
- Application-controlled WebSocket Ping frames in browsers; the native browser
  `WebSocket` API does not expose control-frame Ping/Pong.
...
Every advertised subpath has a matching source barrel and build entry; package
tests import each ESM, CommonJS, and declaration target from the packed artifact.

### Implementation decomposition

The implementation is delivered through a dependency-ordered index and six
independently executable plans. Each plan must typecheck, pass its focused
tests, pass every earlier plan's regression suite, and satisfy a build or
integration gate before the next plan begins:

1. **Codec and package** — package exports, byte message model, headers,
   parser/serializer, and stream framing.
2. **Transport and ingress** — typed transport events, Node/browser adapters,
   framing integration, and the single byte-to-transaction ingress path.
3. **Transactions and dialogs** — exact RFC 3261/RFC 6026 machines, matching,
   timer behavior, both ACK paths, and dialog routing/CSeq state.
4. **Authentication and registration** — digest primitives, challenge retry,
   REGISTER lifecycle, refresh, 423, reconnect, and unregister.
5. **Calls and media** — outgoing/incoming sessions, stub SDP bridge, INVITE
   authentication, ACK/BYE, and a one-call integration flow.
6. **Reliability and release** — liveness strategies, worker recovery,
   tolerance corpus, public exports, package assertions, and the complete
   register-to-call smoke matrix.

The implementation index records these dependencies and the exact interface
each plan consumes. A later plan may not reach around a published interface to
depend on an earlier component's internals.
...
Mutation semantics are explicit: `append(name, value)` adds another field
value, while `set(name, value)` replaces every existing value for that name.
Retry code must use `set` for `Via`, `CSeq`, and authorization fields so old
transaction identifiers and credentials cannot remain serialized.
...
Parsing is byte-oriented. Header delimiters and `Content-Length` boundaries
are located in the input `Uint8Array`; text decoding happens only after byte
boundaries are known. `ParseError.offset` is therefore a byte offset, and a
multibyte UTF-8 body cannot change framing or error positions.
...
- **ACK for a 300–699 response** — generated as part of INVITE transaction
  ...
  sends the ACK, still within the transaction). It reuses the INVITE branch and
  numeric CSeq, changing only the CSeq method to `ACK`.
- **ACK for a 2xx response** — generated by the **UA/dialog layer** as a
  ...
  client transaction. It uses the INVITE numeric CSeq, a new branch, the
  dialog remote target, and the established route set.

The transaction coordinator registers a client transaction under its top Via
branch before the first send, and removes that mapping only on termination.
Every parsed response enters through one ingress coordinator and is matched by
that branch. No caller performs a separate `trackRequest` step.
...
interface TransportCapabilities {
  reliable: boolean;
  framing: 'datagram' | 'stream' | 'message';
}

type TransportEvent =
  | { type: 'connected' }
  | { type: 'data'; data: Uint8Array }
  | { type: 'disconnected'; error?: TransportError }
...
interface Transport {
  ...
  subscribe(callback: (event: TransportEvent) => void): Unsubscribe;
  isConnected(): boolean;
}
```

A transport emits `data` only for a complete SIP message. UDP datagrams and
WebSocket messages already preserve that boundary; the TCP adapter owns a
`SipStreamDecoder`. Immediate send/connect failures reject with
`TransportError`; asynchronous failures use typed `error` or `disconnected`
events. An empty byte array is data, never an error sentinel.
...
**Liveness detection.** The Node adapter can use protocol Ping/Pong through
the injected `ws` implementation. The native browser `WebSocket` API cannot
send Ping control frames, so the browser adapter does not pretend that an
empty data message is a Ping. Browser deployments use an injected
`LivenessStrategy`; v1 supplies an `OPTIONS` strategy that creates an ordinary
non-INVITE SIP transaction and reports a bounded timeout as `TransportError`.
SIP-level CRLF keepalive for UDP/TCP remains a follow-up.

### Ingress coordinator

Exactly one coordinator subscribes to transport events. For each `data` event
it calls the never-throwing parser, reports `ParseError` values through the
typed error surface, and routes valid requests/responses to the transaction
coordinator. It forwards transport failures without converting them into
bytes. User-agent features never subscribe to a socket directly.

### Key design point

The transport does **not** implement transaction timers or retransmission —
that is the transaction layer's job. Node protocol Ping/Pong is transport
maintenance; SIP `OPTIONS` liveness is a UA strategy using a normal
transaction. This keeps the components clean and avoids
...
`unregister()`. Exposes `registerState` events. `register()` resolves only
after a successful final response has established the granted expiry, and
rejects with the typed terminal error after authentication/retry handling.
`unregister()` resolves only after the contact-removal REGISTER succeeds.
...
The retry builder replaces transaction/authentication headers, preserves the
request body and all end-to-end headers, parses complete Digest challenges
without splitting their comma-separated parameters, and tracks retries per
request/nonce rather than globally. Redaction replaces credential values
before any message is exposed to logging.
...
transitions (inviting, ringing, early, confirmed, terminated, failed).

The public operation promises describe protocol outcomes: `invite()` and
`answer()` resolve at confirmed dialog state, `reject()` resolves after the
final response is sent, and `hangup()` resolves after the BYE transaction
succeeds. Terminal failures reject with `SipError` or `TransportError`.
...
- **Liveness detection.** Node WebSocket uses protocol Ping/Pong through the
  injected `ws` object. Browser WebSocket uses the injected SIP `OPTIONS`
  strategy. Both have virtual-clock timeout tests and surface a typed
  `TransportError` instead of passing an empty byte array to the parser.
...
- **Transport:** integration tests against a loopback server (a local SIP server
  ...
  Liveness tests separately verify a Node WebSocket whose Pong is withheld and
  a browser `OPTIONS` request whose response is withheld; both surface
  `TransportError` within the configured timeout. Transport errors are never
  delivered as data.
...
7. Node Ping/Pong and browser SIP `OPTIONS` liveness each surface a dead
   WebSocket connection as `TransportError` within a bounded timeout.
8. A corpus of non-compliant but real-world server messages never crashes the
   parser or takes the stack down.
9. The worker-recovery path (death → respawn → re-register) succeeds with
   correct `Call-ID`/`CSeq` continuity.
10. Every advertised package subpath resolves in both ESM and CommonJS builds
    and has a corresponding declaration entry.
