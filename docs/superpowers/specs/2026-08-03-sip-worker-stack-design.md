...
connect, and RFC 7118 requires each SIP message to occupy exactly one WebSocket
message (no fragmentation, no bundling). The adapter enforces one-message-per-
WebSocket-frame on send and maps each received frame to one message.
...
- **Granted expiry** — honor the `Expires` from the `200` response (not the
  requested value), and refresh before it lapses.
- **Refresh margin** — initiate the refresh at a configured fraction of the
  granted expiry (e.g. half), not at the full expiry.
- **`stale`/rejected expiry** — if the server returns a `423 (Interval Too
  Brief)` or a shorter granted expiry, re-issue with the accepted interval.
...
Flow: send `REGISTER`/`INVITE` unauthenticated → receive the challenge → compute
the digest from the `WWW-Authenticate`/`Proxy-Authenticate` header → retry the
same request as a **new transaction** with a new branch and an
`Authorization`/`Proxy-Authorization` header. For `REGISTER` and `INVITE`,
CSeq handling follows the relevant retry rules. The UA holds the credentials
and performs the retry transparently; the app never sees the challenge dance.
