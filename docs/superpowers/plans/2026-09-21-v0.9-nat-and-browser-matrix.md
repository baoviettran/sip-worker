...
In `docs/ci-browser-gate-notes.md`, replace the `three-engine` line (`:6`) with the row-matrix description: the `rows` job computing the matrix from `test/browser-matrix/rows.mjs`, one `engine` job per row with `timeout-minutes: 40` and a display name equal to the row's `label`, the `publisher` completeness gate, and the nightly `schedule:` (`43 3 * * *`) that runs the vendor rows on top of the PR slice. Record the branch-protection change there too, in the same place a reader looks for which checks are required:
