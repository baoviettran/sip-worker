...
- **The local host cannot verify row behaviour.** The 2026-09-21 spike proved ICE fails for *every* engine here — bundled-current and previous-stable alike (`ice=new`/`failed`, identically), so the failure is environmental, not a driver/build mismatch. Row behaviour and the hardened STUN assertions are verified on the CI runner; every affected task below carries a **Verify at first run** note, in the existing plan idiom.
