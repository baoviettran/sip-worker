...
- `test/browser-media/safari-runner.mjs` already parses `caps.browserVersion` in `createSafariSession` (`:337-341`) and throws when it is `unknown` (`:342`). Recording it costs one module-level binding plus one write in `main`'s `finally` (`:551-569`).
...
- `main().catch` (`:574`) is outside `main`, so the `finally` has run already.
