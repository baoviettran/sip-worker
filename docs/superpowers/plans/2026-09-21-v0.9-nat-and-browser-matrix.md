...
test('the Safari gate runs, always, and cannot be neutralised', () => {
  ...
  // The step that runs the media suite must still run it, and must run even
  // after an earlier step failed — `always()` here is what makes the gate's own
  // exit code the job's verdict.
  const gateStep = [
  ...
  assert.ok(
    ...
    'safari-media.yml must still run its acceptance gate, and always — a skipped gate is a green job with no media coverage',
  );
  ...
  assert.ok(
    !safari.includes('continue-on-error'),
...
| --- | --- |
| change the gate step's `if: always()` to `if: success()` | `must still run its acceptance gate, and always` |
| change `if: always()` to `if: false` | the same rule |
