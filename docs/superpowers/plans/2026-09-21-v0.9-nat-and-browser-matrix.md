...
- Produces:
  ...
  - `renderMatrixDocs(rows): string` — the markdown page body
  - CLI: `node test/browser-matrix/publish.mjs --expected <ids csv> --in <dir> --out <file>` (fails on any missing/mismatched row) and `node test/browser-matrix/publish.mjs --docs [--in <dir>] --out <file>` (with `--in`, the observed column is filled from a merged report; without it, the page renders from the row table and claims no observed version)
