...
`report.mjs` does not exist until T5, so run this with the reporter overridden on the command line:

Run: `MATRIX_EVENT=pr MATRIX_ROWS=chromium-current npx playwright test --config test/browser-matrix/playwright.config.ts --reporter=list test/browser-media/two-way-audio.spec.ts --grep "direct path"`
Expected: the two direct-path tests **fail on ICE** (`ice=new`/`failed`) at the connection assertion — that is the known environmental failure this host cannot avoid, and it proves the row launched, loaded the built bundle, built PCs, and gathered candidates. A row that never launched fails earlier, with `Executable doesn't exist` or an import error; those are the failures to act on here.
