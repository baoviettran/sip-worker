...
Run: `cp test/browser-matrix/launch.ts /tmp/launch.bak && node -e "const f='playwright.config.ts';const fs=require('node:fs');fs.writeFileSync(f,fs.readFileSync(f,'utf8').replace('launchOptions: launchOptionsFor(\'firefox\')','contextOptions: launchOptionsFor(\'firefox\')'))" && npm run test:browsermatrix:integrity 2>&1 | grep -E "contextOptions|firefoxUserPrefs|# fail" | head -5; echo EXIT=${PIPESTATUS[0]}; git checkout playwright.config.ts`
...
function readReports(dir) {
  ...
  for (const name of readdirSync(dir)) {
    if (!name.endsWith('.json') || name === 'matrix.json') continue;
