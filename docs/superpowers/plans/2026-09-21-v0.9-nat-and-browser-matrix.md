...
These read the file as text, like every other rule in the gate — there is no YAML parser in the dependency set, and adding one for this is not worth it. Every rule therefore asserts a contiguous multi-line sequence that occurs exactly once, rather than a loose substring that a comment could satisfy; that is the correction T9's review forced on rule 13, and it is the standard here.
...
| delete the `test-results/browser-matrix/` path entry | `must ship inside the artifact the publication downloads` |
...
If any mutation leaves the gate green, the rule is not load-bearing and the rule — not the mutation — is what needs fixing. Record the five observed failures in your report.
