...
// The runner exits non-zero on the first failed gate; this flag is what lets the
// report it writes in `finally` carry an honest outcome even on the failure path.
...
In the failure paths — the watchdog (`:83-86`) and both `process.exit(1)` sites (`:570-571` and the `main().catch` at `:574`) — set `gateFailed = true;` immediately before exiting.
    ...
    if (safariVersion) {
      try {
        writeReport(buildReport({
          ...
          suites: { 'safari-acceptance': gateFailed || errors.length ? 'failed' : 'passed' },
