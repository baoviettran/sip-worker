...
- `if (errors.length) { … process.exit(1); }` (`:570`) runs *after* `main`'s `finally` (`:551-571`), where the report is written, and by then `errors` is fully populated — the catch above the `finally` pushed into it — so `errors.length` alone already carries the outcome;
...
In `main`'s `finally` (`:551-571`), after the keychain teardown and before the servers close, add:
