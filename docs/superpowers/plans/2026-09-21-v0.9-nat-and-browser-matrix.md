...
- **Branch protection — still to do, and nothing is enforced until it is done.**
  ...
  the required list in repo settings is updated, no required check matches a job
  that runs, so a red matrix does not block a merge.** This is a repository-settings
  change, not a commit — a human makes it, in the same change that edits a row.
...
other cannot fail loudly, which is why it is written down:
...
- that `test-results/browser-matrix/safari-current.json` appears inside the
  ...
  nothing. The Safari row then reads as an absent report downstream, which is the
  right verdict but a hard one to trace back here.
```
