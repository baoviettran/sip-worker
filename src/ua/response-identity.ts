import { extractTag, extractUri } from '../dialogs/header-values.js';
...
export function responseMatchesRequestIdentity(
  ...
  const requestToUri = extractUri(request.headers.get('To'));
  const responseToUri = extractUri(response.headers.get('To'));
  ...
  if (responseToTag === undefined) return false;
  if (requestToTag !== undefined && responseToTag !== requestToTag) return false;
